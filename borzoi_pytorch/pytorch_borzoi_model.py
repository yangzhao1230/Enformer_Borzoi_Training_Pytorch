...
class Borzoi128(PreTrainedModel):
    config_class = BorzoiConfig
    base_model_prefix = "borzoi_128"

    @staticmethod
    def from_hparams(**kwargs):
        return Borzoi128(BorzoiConfig(**kwargs))
    
    
    def __init__(self, config):
        super(Borzoi128, self).__init__(config)
        self.flashed = config.flashed if "flashed" in config.__dict__.keys() else False
        self.enable_human_head = config.enable_human_head if "enable_human_head" in config.__dict__.keys() else True
        self.enable_mouse_head = config.enable_mouse_head   
        self.conv_dna = ConvDna()
        ...
        if self.enable_human_head:
            # self.human_head = nn.Conv1d(in_channels = 1920, out_channels = 7611, kernel_size = 1)
            # 5313 in Enformer
            self.human_head = nn.Conv1d(in_channels = 1920, out_channels = 5313, kernel_size = 1)
        if self.enable_mouse_head:
            self.mouse_head = nn.Conv1d(in_channels = 1920, out_channels = 2608, kernel_size = 1)
        self.final_softplus = nn.Softplus()
    ...
    def get_embs_after_crop(self, x):
        ...
        Returns:
             torch.Tensor: Output of the model up to the cropping layer with shape (N, dim, crop_length)
        """
        x = self.conv_dna(x) # (N, 512, L // 2)
        x_unet0 = self.res_tower(x) # (N, 1280, L // 32)
        x_unet1 = self.unet1(x_unet0) # (N, 1536, L //64)
        x = self._max_pool(x_unet1) # (N, 1536, L // 128)
        x_unet1 = self.horizontal_conv1(x_unet1) # (N, 1536, L // 64)
        x_unet0 = self.horizontal_conv0(x_unet0) # (N, 1536, L // 32)
        x = self.transformer(x.permute(0,2,1))
        # x_128 = x.clone() # (N, L // 128, 1536)
        # x = x.permute(0,2,1) # (N, 1536, L // 128)
        # x = self.upsampling_unet1(x)
        # x += x_unet1
        # x = self.separable1(x)
        # x = self.upsampling_unet0(x)
        # x += x_unet0
        # x = self.separable0(x) # (N, 1536, L // 32)
        # x_128 = self.crop(x_128)

        # x_fine = x.clone()
        # x_fine = x_fine.permute(0,2,1)
        # x_fine = self.upsampling_unet1(x_fine)
        # x_fine += x_unet1
        # x_fine = self.separable1(x_fine)
        # x_fine = self.upsampling_unet0(x_fine)
        # x_fine += x_unet0
        # x_fine = self.separable0(x_fine)
        x_fine = None
        x = self.crop(x)

        return x, x_fine        # return x_fine, x
    ...
    def forward(
        ...
        x = x.permute(0,2,1)
        # x = self.get_embs_after_crop(x)
        x, x_fine = self.get_embs_after_crop(x) # x (N, 1536, L // 32), x_128 (N, L // 128, 1536)
        # x = self.final_joined_convs(x)
        emb = self.final_joined_convs(x.permute(0,2,1))
        # disable autocast for more precision in final layer
        with torch.cuda.amp.autocast(enabled=False):
            # if data_parallel_training:
            #     # we need this to get gradients for both heads if doing DDP training
            ...
            #     else:
            #         return self.final_softplus(self.mouse_head(x.float()))
            # we only use human head

            human_out = self.final_softplus(self.human_head(emb.float())).permute(0,2,1)

        loss = None
        if labels is not None:
            loss = poisson_loss(human_out, labels)
