...
[ -z "${MLP_WORKER_0_HOST}" ] && MLP_WORKER_0_HOST=127.0.0.1 # master的ip
[ -z "${MLP_WORKER_0_PORT}" ] && MLP_WORKER_0_PORT=12345 # master的端口号
[ -z "${MLP_ROLE_INDEX}" ] && MLP_ROLE_INDEX=0 # 当前worker的index
[ -z "${MLP_WORKER_NUM}" ] && MLP_WORKER_NUM=1 # 总worker数量 (Default to 1 if not set)
[ -z "${MLP_WORKER_GPU_NUM}" ] && MLP_WORKER_GPU_NUM=$(nvidia-smi -L | wc -l) # 改成自动获取本机gpu数量
...
export WANDB_PROJECT="Borzoi_Pretrain"
...
run_name="Borzoi_lr_${learning_rate}_bs_${total_batch_size}_len_${seqlen}_rc_${rc_aug}_shift_${shift_aug}"
echo "${run_name}"
output_path="/root/code/${run_name}" # replace with your own output path

torchrun ${DISTRIBUTED_ARGS} train_borzoi.py \
    --run_name ${run_name} \
    --save_steps 5000 \
    --eval_steps 5000 \
    --logging_steps 100 \
